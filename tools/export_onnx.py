"""
Export ChefLM to ONNX format for browser inference via onnxruntime-web.
...
def export_onnx(checkpoint_path, tokenizer_path, output_path, quantize=True, push=False):
    ...
    if os.path.exists(config_json):
        ...
        # to defaults (same bug previously found and fixed in inference.py).
        ...
        config = ChefConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
    else:
        config = ChefConfig()
    ...
    print(f"ChefLM: {total:,} params ({total/1e6:.1f}M)")
    ...
    torch.onnx.export(
        ...
        opset_version=17,
        # As of PyTorch 2.7+, torch.onnx.export defaults to the newer
        # "dynamo" exporter, which requires the separate onnxscript
        # package and unconditionally imports it even when unused here.
        # This call is written for the legacy TorchScript-tracing
        # exporter (dynamic_axes/opset_version style), so pin dynamo=False
        # explicitly rather than depending on whatever the installed
        # torch version's default happens to be.
        dynamo=False,
    )
...
def main():
    parser = argparse.ArgumentParser(description="Export ChefLM to ONNX")
