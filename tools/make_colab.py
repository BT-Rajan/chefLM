...
def build():
    ...
    cells.append(code(
        ...
        "    r = engine.chat_completion([{'role': 'user', 'content': prompt}], max_tokens=64, lang=lang)\n"
        "    return r['choices'][0]['message'].get('content', '').strip()\n"
        "\n"
        "# Test across different topics (English, then a few in Arabic)\n"
        "tests = [\n"
        "    ('what is your favorite milkshake flavor', 'flavor', 'en'),\n"
        "    ('what ingredients go in a milkshake',      'ingredients', 'en'),\n"
        "    ('how do you make a milkshake',              'howto', 'en'),\n"
        "    ('what toppings go well on a milkshake',     'topping', 'en'),\n"
        "    ('should a milkshake be very cold',          'temperature', 'en'),\n"
        "    ('what size milkshake should i order',       'ordering', 'en'),\n"
        "    ('is a milkshake healthy',                    'health', 'en'),\n"
        "    ('is a frappe the same as a milkshake',      'comparison', 'en'),\n"
        "    ('do you think milkshakes are a good dessert', 'opinion', 'en'),\n"
        "    ('when were milkshakes invented',            'funfact', 'en'),\n"
        "    ('ما هي مكونات الميلك شيك',                    'ingredients (ar)', 'ar'),\n"
        "    ('كيف تصنع ميلك شيك',                          'howto (ar)', 'ar'),\n"
        "]\n"
        "\n"
        "print(f'{\"Topic\":<16s}  {\"You\":<40s}  Chef')\n"
        "print('=' * 100)\n"
        "for prompt, topic, lang in tests:\n"
        "    reply = chat(prompt, lang=lang)\n"
        "    print(f'{topic:<16s}  {prompt:<40s}  {reply[:128]}')\n"
    ))
