"""
Export ChefLM to HuggingFace standard format.
...
def export_and_push(checkpoint_path, tokenizer_path, repo_id, token, local_dir="hf_export"):
    ...
    # 5. chef.png — asset
    asset_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "chef.png")
    if os.path.exists(asset_path):
        asset_dir = os.path.join(local_dir, "assets")
        os.makedirs(asset_dir, exist_ok=True)
        import shutil
        shutil.copy2(asset_path, os.path.join(asset_dir, "chef.png"))
        print(f"  Saved assets/chef.png")
...
def main():
    parser = argparse.ArgumentParser(description="Export ChefLM to HuggingFace format")
